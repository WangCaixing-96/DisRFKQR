...
arma::mat ADMM(arma::mat phi_x, arma::vec Y, arma::vec grad_dif, arma::vec r_0, arma::vec u_0, double lambda, double tau, int beta, int S, int max_iter){
  ...
  int iter=0;
  while(iter<max_iter){
    arma::vec theta=((phi_x.t()*phi_x+2*lambda*arma::eye(S,S)).i())*(phi_x.t()*(Y-r_old-u_old)-grad_dif);
    ...
    arma::vec u=u_old+phi_x*theta+r-Y;
    r_old=r;
    u_old=u;

    double epsilon=0;
    double theta_norm=accu(square(phi_x*(theta-theta_old)))/n;
...
arma::mat DKQR_RF(arma::mat phi_x, arma::vec Y, arma::vec r_0, arma::vec u_0, double lambda, double tau, int M, int beta, int S, int max_iter_admm, int max_iter_dis){
  ...
  arma::vec grad_dif_old=dis_grad(phi_x,Y,theta_old,lambda,tau,S,M);
  arma::mat phi_x1=phi_x.rows(0,n-1);
  arma::vec Y1=Y.subvec(0,n-1);
  ...
  for(int i=0;i<max_iter_dis;i++){
    arma::vec grad_dif_i=dis_grad(phi_x,Y,theta_old,lambda,tau,S,M);
    arma::mat para=ADMM(phi_x1,Y1,grad_dif_i,r_old,u_old,lambda,tau,beta,S,max_iter_admm);
    theta_old=para.col(0).subvec(0,S-1);
    r_old=para.col(1).subvec(0,n-1);
    u_old=para.col(2).subvec(0,n-1);
